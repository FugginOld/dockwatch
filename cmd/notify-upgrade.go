// Package cmd contains the dockwatch (sub-)commands
package cmd
...
// NewNotifyUpgradeCommand creates the notify upgrade command for dockwatch
...
func runNotifyUpgradeE(cmd *cobra.Command, _ []string) error {
	...
	outFile, err := os.CreateTemp("/", "dockwatch-notif-urls-*")
	if err != nil {
	...
	sb := strings.Builder{}
	sb.WriteString("DOCKWATCH_NOTIFICATION_URL=")
	...
	logf("cp %v:%v ./dockwatch-notifications.env", containerID, outFile.Name())
	logf("")
