...
function cleanup {
  ...
  pkill -9 -f dockwatch >> /dev/null || true
}
trap cleanup EXIT

DEFAULT_DOCKWATCH="$(dirname "${BASH_SOURCE[0]}")/../dockwatch"
DOCKWATCH=$1
DOCKWATCH=${DOCKWATCH:-$DEFAULT_DOCKWATCH}
echo "dockwatch path is $DOCKWATCH"
...
LABEL com.centurylinklabs.dockwatch.lifecycle.pre-update="cat /opt/test/value.txt"
LABEL com.centurylinklabs.dockwatch.lifecycle.post-update="echo image > /opt/test/value.txt"
...
TMP_DIR="/tmp/dockwatch-commands-test"
mkdir -p $TMP_DIR
...
$DOCKWATCH -i $DOCKWATCH_INTERVAL --no-pull --stop-timeout 2s --enable-lifecycle-hooks $CONTAINER $LINKED_CONTAINER &
sleep 3
...
# Build updated image to trigger dockwatch update
builddocker

WAIT_AMOUNT=$(($DOCKWATCH_INTERVAL * 3))
...
docker run -d -p 0.0.0.0:8888:8888 \
	--label=com.centurylinklabs.dockwatch.lifecycle.post-update="echo container > /opt/test/value.txt" \
