...
Run the `dockwatch` container with the following command:
...
docker run -d \
  ...
  -v /var/run/docker.sock:/var/run/docker.sock \
  fugginold/dockwatch container_to_watch --debug
```
...
Alternatively, if you use 2FA on Docker Hub, passing username and password will be insufficient. Instead, run `docker login` to store your credentials in `$HOME/.docker/config.json` and then mount this config file to make it available to the Dockwatch container:
...
from a private repo on the GitHub Registry and monitors it with dockwatch. Note the command argument changing the interval
