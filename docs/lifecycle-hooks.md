...
**inside** every container updated by dockwatch.
...
`DOCKWATCH_LIFECYCLE_HOOKS` to `true`.
...
| ----------- | ------------------------------------------------------ | 
| Pre Check   | `com.centurylinklabs.dockwatch.lifecycle.pre-check`   |
| Pre Update  | `com.centurylinklabs.dockwatch.lifecycle.pre-update`  | 
| Post Update | `com.centurylinklabs.dockwatch.lifecycle.post-update` |
| Post Check  | `com.centurylinklabs.dockwatch.lifecycle.post-check`  |
...
=== "Dockerfile"
    ```docker 
    LABEL com.centurylinklabs.dockwatch.lifecycle.pre-check="/sync.sh"
    LABEL com.centurylinklabs.dockwatch.lifecycle.pre-update="/dump-data.sh"
    LABEL com.centurylinklabs.dockwatch.lifecycle.post-update="/restore-data.sh"
    LABEL com.centurylinklabs.dockwatch.lifecycle.post-check="/send-heartbeat.sh"
    ```
...
=== "docker run"
    ...
    --label=com.centurylinklabs.dockwatch.lifecycle.pre-update="/dump-data.sh" \
    --label=com.centurylinklabs.dockwatch.lifecycle.post-update="/restore-data.sh" \
    someimage --label=com.centurylinklabs.dockwatch.lifecycle.post-check="/send-heartbeat.sh" \
    ```

### Timeouts
The timeout for all lifecycle commands is 60 seconds. After that, a timeout will
occur, forcing Dockwatch to continue the update loop.
...
label `com.centurylinklabs.dockwatch.lifecycle.pre-update-timeout` or post-update-timeout respectively followed by
...
0 or 75 (EX_TEMPFAIL), will not prevent dockwatch from updating the container. Only an error
