...
func getGotifyToken(flags *pflag.FlagSet) string {
	...
	if len(gotifyToken) < 1 {
		log.Fatal("Required argument --notification-gotify-token(cli) or DOCKWATCH_NOTIFICATION_GOTIFY_TOKEN(env) is empty.")
	}
	return gotifyToken
}
...
func getGotifyURL(flags *pflag.FlagSet) string {
	...
	if len(gotifyURL) < 1 {
		log.Fatal("Required argument --notification-gotify-url(cli) or DOCKWATCH_NOTIFICATION_GOTIFY_URL(env) is empty.")
