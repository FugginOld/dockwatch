...
var _ = Describe("notifications", func() {
	...
	Describe("the slack notifier", func() {
		...
		When("passing a discord url to the slack notifier", func() {
			...
			iconURL := "https://containrrr.dev/dockwatch-sq180.png"
			expected := fmt.Sprintf("discord://%s@%s?color=0x%x&colordebug=0x0&colorerror=0x0&colorinfo=0x0&colorwarn=0x0&username=dockwatch", token, channel, color)
...
func buildExpectedURL(username string, password string, host string, port int, from string, to string, auth string) string {
	var template = "smtp://%s:%s@%s:%d/?auth=%s&fromaddress=%s&fromname=Dockwatch&subject=&toaddresses=%s"
