Dockwatch supports private Docker image registries. In many cases, accessing a private registry
requires a valid username and password (i.e., _credentials_). In order to operate in such an
environment, dockwatch needs to know the credentials to access the registry. 

The credentials can be provided to dockwatch in a configuration file called `config.json`.
...
!!! info "Using private images on Docker Hub"
    ...
    in `docker run` or `docker-compose`. Like Docker, Dockwatch will use the
    Docker Hub registry and its credentials when no registry domain is specified.
    
    <sub>Dockwatch will recognize credentials with `<REGISTRY_NAME>` `index.docker.io`,
...
When the dockwatch Docker container is started, the created configuration file
(`<PATH>/config.json` in this example) needs to be passed to the container:

```bash
docker run [...] -v <PATH>/config.json:/config.json fugginold/dockwatch
```
...
This configuration file can be directly used by dockwatch. In this case, the creation of an
additional configuration file is not necessary.

When the Docker container is started, pass the configuration file to dockwatch:

```bash
docker run [...] -v <PATH_TO_HOME_DIR>/.docker/config.json:/config.json fugginold/dockwatch
```

When creating the dockwatch container via docker-compose, use the following lines:
...
services: 
  dockwatch:
    ...
    volumes:
      - /etc/dockwatch/config/:/config/
      - /var/run/docker.sock:/var/run/docker.sock
```

## Credential helpers
Some private Docker registries (the most prominent probably being AWS ECR) use non-standard ways of authentication.
To be able to use this together with dockwatch, we need to use a credential helper.

To keep the image size small we've decided to not include any helpers in the dockwatch image, instead we'll put the
...
in a volume that may be mounted onto your dockwatch container.
