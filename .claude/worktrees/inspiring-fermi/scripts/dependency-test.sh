...
if [ -z "$1" ] || [ "$1" == "depends-on" ]; then
  DepArgs="--label com.centurylinklabs.dockwatch.depends-on=parent"
elif [ "$1" == "linked" ]; then
  DepArgs="--link parent"
else
  DepArgs=$1
...
if [ -z "$DOCKWATCH_TAG" ]; then
  ## Windows support:
  #export DOCKER_HOST=tcp://localhost:2375
  #export CLICOLOR=1
  go run . --run-once $WatchArgs
else
  docker run -it --rm -v /var/run/docker.sock:/var/run/docker.sock fugginold/dockwatch:"$DOCKWATCH_TAG" --run-once $WatchArgs
fi
